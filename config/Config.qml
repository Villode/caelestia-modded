...
Singleton {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            try {
                ...
                Toaster.toast(qsTr("序列化配置失败"), e.message, "settings_alert", Toast.Error);
            }
        }
    }
    ...
    function serializeLauncher(): var {
        return {
            ...
            customCategories: launcher.customCategories,
            categoryRenamesJson: launcher.categoryRenamesJson,
            appCategoryOverridesJson: launcher.appCategoryOverridesJson,
            sortMode: launcher.sortMode,
            iconSize: launcher.iconSize,
            useFuzzy: {
        ...
        onLoaded: {
            try {
                ...
                if (adapter.utilities.toasts.configLoaded && !recentlySaved && elapsed > 0) {
                    Toaster.toast(qsTr("配置已加载"), qsTr("配置加载耗时 %1ms").arg(elapsed), "rule_settings");
                } else if (adapter.utilities.toasts.configLoaded && recentlySaved && elapsed > 0) {
                    Toaster.toast(qsTr("配置已保存"), qsTr("配置重载耗时 %1ms").arg(elapsed), "rule_settings");
                }
            } catch (e) {
                Toaster.toast(qsTr("加载配置失败"), e.message, "settings_alert", Toast.Error);
            }
        }
        onLoadFailed: err => {
