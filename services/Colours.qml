...
Singleton {
    ...
    function setMode(mode: string): void {
        // Smart mode switch: if current flavour doesn't support the target mode,
        // find a flavour in the same scheme that does
        const schemeName = root.scheme;
        const currentFlavour = root.flavour;

        // Known flavour-mode mappings for schemes where flavour determines mode
        const flavourModes = {
            "catppuccin": { "latte": "light", "frappe": "dark", "macchiato": "dark", "mocha": "dark" },
            "rosepine": { "dawn": "light", "main": "dark", "moon": "dark" }
        };

        const mapping = flavourModes[schemeName];
        if (mapping && mapping[currentFlavour] !== mode) {
            // Need to switch flavour too
            let targetFlavour = "";
            // Prefer mocha for dark catppuccin, main for dark rosepine
            const darkDefaults = { "catppuccin": "mocha", "rosepine": "main" };
            const lightDefaults = { "catppuccin": "latte", "rosepine": "dawn" };
            const defaults = mode === "dark" ? darkDefaults : lightDefaults;

            if (defaults[schemeName]) {
                targetFlavour = defaults[schemeName];
            } else {
                for (const [flav, m] of Object.entries(mapping)) {
                    if (m === mode) { targetFlavour = flav; break; }
                }
            }

            if (targetFlavour) {
                Quickshell.execDetached(["caelestia", "scheme", "set", "--notify", "-f", targetFlavour, "-m", mode]);
                return;
            }
        }
