...
Singleton {
    ...
    readonly property string kbLayoutFull: keyboard?.activeKeymap ?? "未知"
    ...
    onCapsLockChanged: {
        ...
        if (capsLock)
            Toaster.toast(qsTr("大写锁定已启用"), qsTr("大写锁定当前已启用"), "keyboard_capslock_badge");
        else
    ...
    onNumLockChanged: {
        ...
        if (numLock)
            Toaster.toast(qsTr("数字锁定已启用"), qsTr("数字锁定当前已启用"), "looks_one");
        else
    ...
    onKbLayoutFullChanged: {
        if (hadKeyboard && Config.utilities.toasts.kbLayoutChanged)
            Toaster.toast(qsTr("键盘布局已更改"), qsTr("布局已切换为: %1").arg(kbLayoutFull), "keyboard");
