            ...
            ColumnLayout {
                ...
                SectionContainer {
                    ...
                    PropertyRow {
                        showTopMargin: true
                        label: qsTr("信号强度")
                        value: root.network ? qsTr("%1%").arg(root.network.strength) : qsTr("不适用")
                    }

                    PropertyRow {
                        showTopMargin: true
                        label: qsTr("频率")
                        value: root.network ? qsTr("%1 MHz").arg(root.network.frequency) : qsTr("不适用")
                    }

                    PropertyRow {
                        showTopMargin: true
                        label: qsTr("安全性")
                        value: root.network ? (root.network.isSecure ? root.network.security : qsTr("打开")) : qsTr("不适用")
                    }
                }
            }
        },
