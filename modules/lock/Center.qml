        ...
        StyledText {
            ...
            readonly property string msg: {
                ...
                if (pam.state === "max" && pam.fprintState === "max")
                    return qsTr("密码和指纹尝试次数均已达上限。");
                if (pam.state === "max") {
                    if (pam.fprint.available)
                        return qsTr("密码尝试次数已达上限。请使用指纹。");
                ...
                if (pam.fprintState === "fail")
                    return qsTr("指纹未识别 (%1/%2)。请重试或使用密码。").arg(pam.fprint.tries).arg(Config.lock.maxFprintTries);
